...
window.location.href=f},null,null,false);return null
...
this.$B=5;this.$c(ServerModel.templateId,a);this.$q.openTest(b,c,this.$B,ss.mkdel(this,function(d){this.$f(d,null)
...
$.ajax(c,{})},save:function(a,b,c,d,e,f){this.$q.save(a,b,ServerModel.userId,ServerModel.localeId,c,d,e,f)
...
c=this.getSetting(d,"");if(c===""){var e;if(a===0){switch(VEL.Wordwall.Global.Server.Switches.getInteger(VEL.Wordwall.Global.Server.Switches.static$1,"Music_Default_Mute_Setting")){case 1:e=false;
...
d.set_guid(a);return d}return null},save:function(a,b,c,d,e,f,g,h){this.$c=true;
if(!ss.isNullOrEmptyString(g)){this.$9.get_document().set_title(g)
...
}this.$8(a,b,function(i,j){ServerModel.activityGuid=j;
e(i,j)},f,h)},saveOptions:function(a,b){var c=this.$9.get_document();
...
b(h)}}else{c()}};d.send()},$8:function(a,b,c,d,e){var f=this.$9.get_document().get_guid();
var g=this.$9.get_document().save();var h="/createajax/upload?guid="+f+"&activityId="+a+"&userId="+this.$9.get_document().get_userId()+"&isAIGenerated="+e;
if(ss.isValue(b)){h+="&folderId="+b}h+="&screenshot="+VEL.Wordwall.Global.Helpers.SettingsHelper.get("enable_screenshot",true);
h+="&originalActivityId="+((ServerModel.originalActivityId===undefined)?0:ServerModel.originalActivityId);
var i=new Uint8Array(g.byteLength);for(var j=0;j<g.byteLength;
j++){i[j]=g.getUint8(j)}var v=new XMLHttpRequest();
v.open("POST",h,true);v.onreadystatechange=function(u){if(v.readyState===4){if(v.status===200){var x=JSON.parse(v.responseText);
VEL.Wordwall.Global.Helpers.AutoSaveHelper.clear();
c(x.ActivityId,x.ActivityGuid)}else{if(!ss.staticEquals(d,null)){d()
}}}};v.setRequestHeader("Content-Type","application/octet-stream");
v.send(i)},$3:function(a,b,c,d){if(a.get_guid()==="00000000000000000000000000000000"){var e=window.localStorage["test.config"];
